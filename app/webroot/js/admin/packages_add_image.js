...
multiupload = {
	init: function() {
        ...
        uploader.bind('FileUploaded', function(up, file, info) {
			...
			//create gallery
			multiupload.createThumbnail(file.id, file.name, '#packageImageGallery'); 
	...
	scriptCreator: function(id, name, divClass){
		$(".thumbnail[imageId='"+id+"']").click(function(){		
			...
			var filename =  id +'.'+name.split(".").pop();
			var primary_input = '<input type="hidden" package="'+parent_id+'" filename="'+id+'" name="data['+parent_id+'][image_main]" value="'+filename+'"/>';
			//remove other instances
			$("#imagesFormDiv input[package='"+parent_id+"']").remove();
		...
		$("#removeImage-"+id).click(function(){
			...
			$("#accordion4 .package_images #thumbnail-"+id).each(function(){
				$(this).remove();
			});
		});

...
public class PrintPostScript implements Printable {
    ...
    private final AtomicReference<Reflect> pdfFile = new AtomicReference<Reflect>(null);
    ...
    public void print() throws PrinterException {
        ...
        if (this.bufferedImage.get() != null) {
            ...
            h = ((Float)getPDFFile().call("getPage", 1).call("getWidth").get()).intValue();
        } else {
    ...
    /**
     * Render the PDF to a graphical image using PDFRenderer and send it's contents
     * to the printer.
     * @param graphics
     * @param pageFormat
     * @param pageIndex
    ...
    private int printPDFRenderer(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
        /*
         * Important:  Since 1.6.7, This uses class reflection to instantiate 
         * and invoke PDFRenderer to reduce reliance on the project's classpath
        ...
        Rectangle2D pageBBox = (Rectangle2D)page.call("getBBox").get();
        
        Reflect pgs = Reflect.on("com.sun.pdfview.PDFRenderer").create(page.get(), g2, pageBox.getBounds(), pageBBox, Color.WHITE);
        
        try {
            page.call("waitForFinish");
            pgs.call("run");
        } catch (Throwable t) {
            LogIt.log(Level.WARNING, "Error rendering PDF", t);
        }
        ...
        if (pdf == null || pageIndex < 1 || pageIndex <= pdf.getNumPages()) {
            ...
            // it the PDFPage into the printing area
            Graphics2D g2 = (Graphics2D) graphics;
...
private int printImage(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
    ...
    /**
    // Future image features (Thanks Anton)
    ...
    return new Dimension(bufferedImage.get().getWidth(null), bufferedImage.get().getHeight(null));
    }
     ...
     */
    private Reflect getPDFFile() throws PrinterException {
        //pdfFile.set(Reflect.on("com.sun.pdfview.PDFFile").create());
        
        if (pdfFile.get() == null && bufferedPDF.get() != null) {
            try {
                pdfFile.set(Reflect.on("com.sun.pdfview.PDFFile").create(this.bufferedPDF.get()));
            } catch (Exception e) {
                throw new PrinterException(e.getMessage());
            }
        }

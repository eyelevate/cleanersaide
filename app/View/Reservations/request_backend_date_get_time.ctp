		...
		if($results == 'empty'){
			...
			foreach ($results as $key => $value) {
				...
				foreach ($results[$key] as $akey => $avalue) {
					...
					$reserved = $results[$key][$akey]['reserved'];
					$reservable_units = $results[$key][$akey]['reservable_units'];
					$total_reserved = '['.$reserved.'/'.$reservable_units.']';
				}
			?>
			<tr <? if (strpos($label,'success') !== false) {?>class="pointer touch"<? } else {?>class="noTouch"<?}?> schedule_id ="<?php echo $schedule_id;?>" inventory_id="<?php echo $inventory_id;?>" >
				...
				<td><?php echo $label.' '.$total_reserved;?></td>
			</tr>
			<?php
			}
		}

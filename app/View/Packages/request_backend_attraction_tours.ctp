...
if($_REQUEST['attraction_id'] && $_REQUEST['start']){
	...
	if(count($tour_rates)>0){
		...
		if($time > 0 && $notime == 0 || $time > 0 && $notime > 0){
			...
			?>
			<li id="timedToursDiv" class=" well well-large clearfix" style="padding-right:0px; padding-left:0px;">
	...
	if(count($tour_rates)>0){
					...
					switch($time_tour){
						case 'Yes':
							foreach ($tour_inventory as $tinKey => $tinValue) {
								...
								<li id="tourLi-<?php echo $tour_id;?>" time="<?php echo $tour_time;?>"  class="tourLi clearfix" <?php echo $tour_hide.$timedLi;?> title="<?php echo $title;?>" style="border-bottom:2px dashed #5e5e5e">
									...
									<div class="clearfix" >
														...
														if($tour_time == $time){
															foreach ($tour_inventory[$tiKey] as $t) {
																...
																<div class="control-group pull-left span3">
																	...
																	<div class="hide">
																		<input name="data[Attraction_reservation][purchase_info][<?php echo $age_range;?>][name]" value="<?php echo $age_range;?>" type="hidden"/>
																		<input name="data[Attraction_reservation][purchase_info][<?php echo $age_range;?>][inventory]" value="<?php echo $inventory;?>" type="hidden"/>
																		<input name="data[Attraction_reservation][purchase_info][<?php echo $age_range;?>][gross]" value="<?php echo $gross;?>" type="hidden"/>
																	</div>	
																</div>
																<?php									
																}										
										...
										<div class="">
											<ul class="unstyled">
												...
												<li id="summaryLi" class="span6 clearfix">
													<table id="summaryTable" class="table table-borderd table-condensed table-hover hide">
														<thead>
															<tr>
																<th>Item</th>
																<th>Total</th>
														...
														<tfoot>
															<tr>
																<td style="border-top:1px solid #000000">Total Pretax</td>
																<td id="total_pretax" style="text-align:right; border-top:1px solid #000000"></td>
															</tr>
															<tr>
																<td>Total Tax</td>
															...
															<tr>
																<td style="border-top:1px solid #000000"><strong>Total Due</strong></td>
																<td id="total_after_tax" style="text-align:right; font-weight:bolder; border: 1px solid #000000;"></td>
															</tr>
														</tfoot>
													</table>
												</li>
												<li style="padding:0px; margin-bottom:10px;">
													<button id="bookTour-<?php echo $tour_id;?>" class="bookTour <?php echo $disabled;?> btn btn-success btn-block" type="button" time="<?php echo $time;?>"  date="<?php echo $date;?>" <?php echo $disabled;?>>SELECT TOUR</button>
												</li>
											</ul>
										</div>						
									</div>
									...
									<div class="hide">
										...
										<input type="hidden" value="<?php echo $tour_time_check;?>" name="data[Attraction_reservation][time]"/>
									</div>
								</li>						
								<?php
							}	
						...
						default:
							...
							?>
								...
								<li id="tourLi-<?php echo $tour_id;?>"   class="tourLi clearfix" <?php echo $tour_hide.$timedLi;?> title="<?php echo $title;?>" style="border-bottom:2px dashed #5e5e5e">
													...
													if($time_tour == 'Yes'){
														if($tour_time == $time){
															foreach ($tour_inventory[$tiKey] as $t) {
																...
																<div class="control-group pull-left span3">
																	<label><?php echo $age_range;?></label>
																	<input type="text" class="typePrice span12" age_range="<?php echo $age_range;?>" reserved="<?php echo $reserved;?>" inventory="<?php echo $inventory_limit;?>" tax_rate="<?php echo ($tax_rate/100); ?>" gross="<?php echo $gross;?>"/>
																	<span class="help-block"></span>
																	<!-- purchase info for form-->
																	<div class="hide">
																		<input name="data[Attraction_reservation][purchase_info][<?php echo $age_range;?>][name]" value="<?php echo $age_range;?>" type="hidden"/>
												...
												<li id="summaryLi" class="span6 clearfix">
													<table id="summaryTable" class="table table-borderd table-condensed table-hover hide">
														...
														<tfoot>
															...
															<tr>
																<td style="border-top:1px solid #5e5e5e"><strong>Total Due</strong></td>
																<td id="total_after_tax" style="text-align:right; font-weight:bolder; border: 1px solid #000000;"></td>
															</tr>
														</tfoot>
													</table>
									...
									<div class="hide">
										<input type="hidden" value="<?php echo $tour_id;?>" name="data[Attraction_reservation][tour_id]"/>
										<input type="hidden" value="No" name="data[Attraction_reservation][time]"/>
									</div>
								</li>						
								<?php	
						break;
		...
		?>
		<li class=""><strong class="text text-error">There are no tours on this selected date. Please select another date!</strong></li>
		<?php
	}		
}
?>
...
foreach ($room_rates as $room) {
	...
	foreach ($packages as $p) {
		...
		foreach ($package_rooms as $prkey => $orValue) {
			if($room_id == $prkey):
			?>
			<div id="roomLi-<?php echo $room_id;?>" class="roomLi twelve columns alpha" style="padding:15px 0; border-bottom: 1px dashed #aaa;" >
				<a class="three columns alpha colorbox">
					<img class="media-object img-border" src="<? echo $room_primary_image; ?>" style="width:150px;"/>
				</a>
				
					<div id="hotelRoomAvailabilityDiv" class="six columns">
						...
						if(count($rates)>0){
							...
							foreach ($rates as $rkey=>$rvalue) {
								...
								$plus_minus_gross = sprintf('%.2f',round($gross - $lowest_room_rate,2));
								...
								if($new_reserved > $total_rooms || ($adults + $children) > ($room_count * $occupancy_max)) {
									...
									$code = $generate_code.$last_digit;
									$available++;
									$text_color = 'error';
									$price_text = '';
									$available_text = '<label class="availability" code="'.$code.'">Not Available</label>';
									$count_not_available++;
								} else {
									$last_digit = 0;
									$generate_code = rand(1000,9999);
									...
									$available_text = 'Status: <label class="label label-success availability" code="'.$code.'">Available</label>';
									$text_color = '';
								}							
								if($bs_count > 0){
								...
								if($check_in < $cutoff){
									...
									$text_color = 'error';	
									$count_not_available++;
									$available_text = '<label class="availability" code="'.$code.'">Past Cutoff</label>';
					...
					<div class="three columns omega">
						<ul class="roomSelectionUl">
							<li class="roomPlusMinusLi"><?php echo $edited_gross;?></li>
							<li>
